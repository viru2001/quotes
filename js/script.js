...
const getQuotes = async () => {
  try {
    ...
    const response = await fetch("https://goquotes-api.herokuapp.com/api/v1/random?count=1000", setHeader);
    const result = await response.json();
    // console.log(result);
    // console.log(result['quotes']);
    let totalQuotes = result['count'];
    let i;
    for (i = 0; i < totalQuotes; i++) {
      data.push({ "text": result['quotes'][i]['text'], author: result['quotes'][i]['author'] })
    }
    //  console.log(data);
    displayQuotes();
...
const displayQuotes = () => {
  ...
  if (count == 0) {
    ...
    colorToUse = colors[Math.floor((Math.random() * colors.length))];
  }
